...
for (minsup_border in minsup_borders){
  for (x in 1:top_n) {
    set <- read.csv(paste0("../results/aumann/mimic/aumann__los__",x,"__",minsup_border,"__set","/part-00000"), header=FALSE)
    colnames(set) <- "LOS"
    set <- set %>% select(LOS)
    
    set_grouped <- set %>% group_by(LOS) %>% count()
    set_grouped$rel <- set_grouped$n / sum(set_grouped$n)#
    set_mean <- mean(set$LOS)
    set_sd <- sd(set$LOS)
    
    
    counter_set <- read.csv(paste0("../results/aumann/mimic/aumann__los__",x,"__",minsup_border,"__counter_set","/part-00000"), header=FALSE)
    colnames(counter_set) <- "LOS"
    counter_set <- counter_set %>% select(LOS)
    ...
    counter_set_mean <- mean(counter_set$LOS)
    counter_set_sd <- sd(counter_set$LOS)
    
    
    pdf(paste0("../results/aumann/mimic/mimic__aumann__Fig__",x,"__",minsup_border,".pdf"), width = 12, height = 12)
