...
for (minsup_border in minsup_borders){
  for (x in 1:top_n) {
    ...
    set <- read.csv(paste0("../results/scenario1/mimic/los__",x,"__",minsup_border, "__set"))
    
    set$LOS <- as.integer(set$LOS)   
    set <- set %>% group_by(LOS) %>% count()
    set$rel <- set$n / sum(set$n)
    
    counter_set <- read.csv(paste0("../results/scenario1/mimic/los__",x,"__",minsup_border,"__counter_set"))
