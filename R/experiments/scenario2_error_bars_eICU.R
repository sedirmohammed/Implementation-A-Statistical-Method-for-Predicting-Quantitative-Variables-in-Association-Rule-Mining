...
for (minsup_border in minsup_borders){
  for (x in 1:top_n) {
    ...
    for (resample in 1:10000) {
      ...
      set <- read.csv(paste0("../results/scenario2/eICU/icd10/los__",x,"__",minsup_border,"__set","/part-00000"))
      colnames(set) <- "LOS"
      ...
      counter_set <- read.csv(paste0("../results/scenario2/eICU/icd10/los__",x,"__",minsup_border,"__counter_set","/part-00000"))
    ...
    print(paste0("Error bars scenario 2 los_", x, "__", minsup_border, ": ", kld_sd))
  }
}
...
for (minsup_border in minsup_borders){
  for (x in 1:top_n) {
    ...
    colnames(set) <- "LOS"
    set$LOS <- as.integer(set$LOS)
    print(paste0("Mean scenario 2 los_", x, "__", minsup_border, ": ", set$LOS %>% mean()))
    print(paste0("SD scenario 2 los_", x, "__", minsup_border, ": ", set$LOS %>% sd()))
    print(paste0("IQR scenario 2 los_", x, "__", minsup_border, ": ", set$LOS %>% IQR()))
    print(paste0("SUP scenario 2 los_", x, "__", minsup_border, ": ", set$LOS %>% length()))
    print(paste0(""))
  }
}
