...
def calculateExpoKLUDF = udf((p: Double, q: Double) => {
...
def calculateLambdaUDF = udf((itemset: mutable.Seq[Int]) => {
...
while(true){
	...
	rules = previousFrequents
		...
		.withColumn("KL_setExpo", calculateKLconfidenceUDF($"LOS_values"))
		.withColumn("KL_counterSetExpo", calculateKLconfidenceUDF($"counter_set_LOS_values"))
...
//all_rules = all_rules.withColumn("rounded_score", round(col("counter_set_KL"), 4)).filter($"KL_setExpo" <= 0.15).filter($"KL_counterSetExpo" <= 0.15).orderBy(desc("counter_set_KL")).cache


all_rules.withColumn("rounded_score", round(col("counter_set_KL"), 4)).filter($"KL_setExpo" <= 0.15).filter($"KL_counterSetExpo" <= 0.15).orderBy(desc("counter_set_KL"))
	.withColumn("lambda_set", calculateLambdaUDF($"LOS_values"))
	.withColumn("lambda_counterSet", calculateLambdaUDF($"counter_set_LOS_values"))
	.withColumn("KL_BetweenExpo", calculateExpoKLUDF($"lambda_counterSet", $"lambda_set"))
	.orderBy(desc("KL_BetweenExpo"))
	.show(2, true)
