...
var df = spark.read.option("header", true).option("inferSchema", true).csv("/home/sedirmohammed/dev/dr_rer_medic/data/telco_customer_churn/telco_customer_churn.csv").cache()
...
frequent_itemsets.select($"items", $"mean").limit(10).rdd.repartition(1).map(_.toString().replace("WrappedArray(","").replace(" ", "").replace("),", " $\\Rightarrow$ ").replace(","," $\\wedge$ ").replace("[","").replace("]", "")).saveAsTextFile("/home/sedirmohammed/dev/dr_rer_medic/results/aumann/telco_customer_churn/latex__tenure__" + MINSUP)


// distributions saving
frequent_itemsets.select($"quant_values").limit(1).rdd.repartition(1).map(_.toString().replace("WrappedArray(","").replace(")", "").replace("[","").replace("]", "").replace(" ", "").replace(",", "\n")).saveAsTextFile("/home/sedirmohammed/dev/dr_rer_medic/results/aumann/telco_customer_churn/aumann__tenure__1__" + MINSUP + "__set")
frequent_itemsets.select($"counter_quant_values").limit(1).rdd.repartition(1).map(_.toString().replace("WrappedArray(","").replace(")", "").replace("[","").replace("]", "").replace(" ", "").replace(",", "\n")).saveAsTextFile("/home/sedirmohammed/dev/dr_rer_medic/results/aumann/telco_customer_churn/aumann__tenure__1__" + MINSUP + "__counter_set")

frequent_itemsets = frequent_itemsets.withColumn("id", monotonically_increasing_id()).filter($"id" > 0).drop("id")
frequent_itemsets.select($"quant_values").limit(1).rdd.repartition(1).map(_.toString().replace("WrappedArray(","").replace(")", "").replace("[","").replace("]", "").replace(" ", "").replace(",", "\n")).saveAsTextFile("/home/sedirmohammed/dev/dr_rer_medic/results/aumann/telco_customer_churn/aumann__tenure__2__" + MINSUP + "__set")
frequent_itemsets.select($"counter_quant_values").limit(1).rdd.repartition(1).map(_.toString().replace("WrappedArray(","").replace(")", "").replace("[","").replace("]", "").replace(" ", "").replace(",", "\n")).saveAsTextFile("/home/sedirmohammed/dev/dr_rer_medic/results/aumann/telco_customer_churn/aumann__tenure__2__" + MINSUP + "__counter_set")
